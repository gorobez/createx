(() => {
    ...
    da.init();
    function timerModule(timerlector) {
        ...
        function countdownTimer() {
            if (document.querySelector(timerlector)) {
    ...
    const Timer = timerModule;
    function activeLinkModule() {
        let link = document.getElementsByClassName("menu__link");
        let url = document.location.href;
        for (let i = 0; i < link.length; i++) if (url == link[i].href) link[i].classList.add("active-link");
    }
    const Activelink = activeLinkModule;
    function initializeFilterModule({buttonSelector, cardsSelector}) {
        const buttons = document.querySelectorAll(buttonSelector);
        const cards = document.getElementsByClassName(cardsSelector);
        ...
        function filterCount() {
            ...
            for (const card of cards) {
                ...
                for (const category of categories) if (category !== cardsSelector) categoryCounts[category] = (categoryCounts[category] || 0) + 1;
            }
            buttons.forEach((button => {
                const span = document.createElement("span");
                const category = button.dataset.filter.toLowerCase();
                const count = "all" === category ? cards.length : categoryCounts[category] || 0;
    ...
    const Filter = initializeFilterModule;
    window.addEventListener("DOMContentLoaded", (function() {
        Timer(".timer");
        Activelink();
        Filter({
            cardsSelector: "courses-online-page__item",
            buttonSelector: ".categories-courses-online__item"
        });
    }));
    window["FLS"] = false;
    isWebp();
